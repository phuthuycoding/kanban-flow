...
- [ ] **Ba skill chưa biết tới danh sách context khai báo.** `kanban-brainstorm` và `kanban-flow` vẫn chỉ dạy `kf new --context {ctx}` và luật ký tự, không nhắc danh sách khai báo, không nhắc việc bị từ chối, không nhắc `kf contexts`. `kanban-brainstorm` đúng là thứ agent đọc ngay trước khi đâm vào rào chắn mới, mà nó không có hướng dẫn phục hồi. Nằm ngoài TASK-007 của đợt `context-registry` nên để lại.
...
- [ ] **`harness-roles.test.ts` flake khi máy tải nặng.** Bốn test trong file này spawn worker process thật và mất 700–1900ms mỗi cái; `vitest.config.ts` không đặt `testTimeout` nên chúng chạy với mặc định 5000ms. Chạy suite bốn lượt liên tiếp thì lượt thứ tư fail đúng bốn test, tất cả ở 5025ms — timeout, không phải sai logic. Có sẵn từ `8551253` (đợt agent-roles), không do `context-registry` hay `cancelled-item-never-validates` gây ra. CI hiện vẫn xanh trên Node 20/22 nhưng đây là loại lỗi chỉ hiện khi runner chậm. Sửa: đặt `testTimeout` riêng cho các file spawn process, đừng nâng toàn cục.
- [ ] **`listFeatures` chết vì symlink gãy dưới `.works/`.** `features.ts:212` gọi `statSync` không bảo vệ, nên `kf contexts`, `kf new`, `kf list`, `kf view` đều ném `ENOENT` thô. Lỗi có sẵn, không do đợt `context-registry`, nhưng `kf contexts` là lệnh mới và thừa hưởng. `kf init` đã miễn nhiễm vì nó không đọc metadata nữa.
