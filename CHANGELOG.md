...
### Fixed
- **`kf status` giấu đúng thứ đang chặn.** Nó chỉ duyệt danh sách artifact, nên một item có thể in `Artifacts: 5/5` và `Next: … kf approve`, rồi `kf approve` fail ngay với bốn lỗi traceability mà status chưa từng nhìn tới. Tệ nhất là với worker tự động: prompt của harness bảo **mọi** worker chạy `kf status` để lấy checklist. Nay status mang theo danh sách lỗi chặn lấy thẳng từ `validateFeature` — không luật nào bị viết lại — và không còn gợi ý một lệnh sắp bị từ chối.
