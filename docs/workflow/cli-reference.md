...
| Lệnh | Tác dụng |
| --- | --- |
| `kf install [--agent <id> ...]` | Cài 8 skill vào skill directory cấp user; mặc định `claude`. Agent hỗ trợ: `claude`, `codex`, `gemini`, `kiro`, `cursor`, `opencode`. |
| `kf uninstall [--agent <id> ...]` | Gỡ đúng 8 skill do kaban-flow quản lý, không xóa skill khác. |
