...
| `kanban-implement` | Thực thi task trong contract, giữ scope, cập nhật code/test; subagent chạy theo prompt contract (task/files/acceptance/constraints) + status protocol | Không mở rộng scope hoặc bỏ qua plan mà không quay lại planning |
| `kanban-test` | Chạy test, ghi execution ID và evidence, phân loại PASS/FAIL/REJECT/BLOCKED | Không biến BLOCKED thành PASS |
| `kanban-review` | Review implementation, test, architecture, security và scope — gồm AI-risk lens (phantom tests, catch-and-swallow, scope drift) | Không archive khi report không PASS |
